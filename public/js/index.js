...
async function handleLogin(event) {
    ...
    try {
        ...
        if (data.success) {
            ...
            carouselManager.initialize(); 
            loadHeroStats();
            showNotification(`¡Bienvenido de vuelta, ${data.data.user.username}!`, 'success');
        } else {
            showAuthMessage('login-message', data.message || 'Error: Credenciales inválidas', 'error');
        }
    } catch (error) {
        showAuthMessage('login-message', 'Error de conexión con el servidor.', 'error');
    }
...
async function handleRegister(event) {
    ...
    try {
        ...
        if (data.success) {
            showAuthMessage('register-message', '¡Registro exitoso! Iniciando sesión...', 'success');
            ...
            showAuthMessage('register-message', data.message || 'No se pudo crear la cuenta', 'error');
        }
    } catch (error) {
        showAuthMessage('register-message', 'Error de conexión con el servidor.', 'error');
    }
...
async function loadHeroStats() {
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('stats-reviews').textContent = data.data.interactions || 0;
        }
    } catch (error) {
...
function toggleProfileDropdown() {
    if (getToken()) { // Solo funciona si está logueado
...
class CarouselManager {
    constructor() {
        this.carousels = {
            popular: { position: 0, maxPosition: 0, cardsPerView: 0, cardWidth: 0, totalCards: 0 },
            recommended: { position: 0, maxPosition: 0, cardsPerView: 0, cardWidth: 0, totalCards: 0 }
        };
    ...
    createBookCard(book) {
        ...
        const stars = Array.from({ length: 5 }, (_, i) => {
            return `<span class="star ${i < Math.floor(rating) ? '' : 'empty'}">★</span>`;
        }).join('');
    ...
    async initializeCarousel(type) {
        ...
        try {
            ...
            if (error.message !== 'No autorizado') { // No mostrar error si es solo por no estar logueado
        ...
        if (books.length === 0) {
            carouselElement.innerHTML = `<p style="padding: 1rem; color: var(--text-secondary); text-align: center;">${type === 'recommended' ? '¡Bienvenido! Valora libros para recibir recomendaciones.' : 'No hay libros para mostrar.'}</p>`;
            return;
        }

        carouselElement.innerHTML = books.map(book => this.createBookCard(book)).join('');
    ...
    calculateDimensions(type) {
        ...
        if (this.isMobile) {
            ...
            const wrapper = document.querySelector(`#${type}-carousel`).closest('.carousel-wrapper');
            const wrapperWidth = wrapper.offsetWidth - 160;
            carousel.cardsPerView = Math.max(3, Math.floor(wrapperWidth / carousel.cardWidth));
            if (window.innerWidth >= 1920) {
                carousel.cardsPerView = Math.max(4, Math.floor(wrapperWidth / carousel.cardWidth));
            }
        }
        carousel.maxPosition = Math.max(0, carousel.totalCards - Math.floor(carousel.cardsPerView));
    }
    ...
    createIndicators(type) {
        ...
        const carousel = this.carousels[type];
        const totalPages = Math.ceil(carousel.totalCards / Math.floor(carousel.cardsPerView));
    ...
    updateCarousel(type) {
        ...
        if (this.isMobile) {
            const track = carouselElement.parentElement;
            track.scrollTo({ left: carousel.position * carousel.cardWidth, behavior: 'smooth' });
        } else {
    ...
    updateNavigationButtons(type) {
        ...
        if (nextBtn) nextBtn.classList.toggle('disabled', carousel.position >= carousel.maxPosition);
    }
    ...
    move(type, direction) {
        ...
        carousel.position = Math.max(0, Math.min(carousel.position, carousel.maxPosition));
        this.updateCarousel(type);
        this.updateIndicators(type);
    }
...
async function performSearch(searchTerm) {
    ...
    try {
        const data = await fetchWithAuth(`/books/search?q=${encodeURIComponent(searchTerm)}&limit=10`);
        if (data.success) {
...
function displaySearchResults(results, searchTerm) {
    ...
    dropdown.innerHTML = results.map(book => createSearchResultItem(book, searchTerm)).join('');
}

function createSearchResultItem(book, searchTerm) {
    const rating = book.averageRating || 0;
    const stars = Array.from({ length: 5 }, (_, i) => `<span class="star ${i < Math.floor(rating) ? '' : 'empty'}">★</span>`).join('');
        ...
        <div class="search-result-item" onclick='selectSearchResult("${book._id}")'>
            ...
            <div class="search-result-info">
                ...
                <div class="search-result-rating">
                    ...
                    <span class="search-result-number">${rating.toFixed(1)}</span>
                </div>
            </div>
        </div>
    `;
...
function highlightSearchTerm(text, searchTerm) {
    ...
    return text.replace(regex, '<mark style="background: rgba(59, 130, 246, 0.3); color: inherit; padding: 0 2px; border-radius: 3px;">$1</mark>');
}
...
async function openModal(bookId) {
    if (!getToken()) {
        showNotification('Debes iniciar sesión para ver los detalles del libro.', 'error');
        showLogin();
        return;
    }
...
function updateReviewsDisplay(average, total, breakdown) {
    ...
    document.getElementById('total-reviews').textContent = `basado en ${total} reseñas`;
...
function loadUserRating(bookId) {
    // TODO: La API debería devolver la valoración actual del usuario.
...
async function showRecommendations(bookId) {
    ...
    try {
        const data = await fetchWithAuth(`/recommendations/content/${bookId}?limit=3`);
        if (data.success && data.data.recommendations.length > 0) {
            const recommendations = data.data.recommendations;
            grid.innerHTML = recommendations.map(rec => createRecommendationItem(rec.book)).join('');
        } else {
